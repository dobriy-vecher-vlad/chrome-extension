...
const getUserIcons = (data, embedded = false) => {
	...
	return (<>
		{isVerified&&getTextTooltip(<Icon16Done/>, 'Доверенное и подтверждённое лицо', embedded)}
		{isBad&&getTextTooltip(<Icon16Cancel/>, 'Имеет дело в суде и неадекватное поведение', embedded)}
		{isAdmin&&getTextTooltip(<Icon16Flash/>, 'Разработчик и администратор скрипта', embedded)}
		{isGeneral&&getTextTooltip(<Icon16User/>, 'Генерал своей гильдии', embedded)}
		{isLeader&&getTextTooltip(<Icon16Crown/>, 'Лидер своей гильдии', embedded)}
	</>);
};
...
const getUserCard = (user) => (<SimpleCell disabled before={user.avatar?<Avatar src={`${pathImages}bot/arena/avatar_${user.avatar}.png`} mode="app" size={48}/>:<InitialsAvatar mode="app" gradientColor={calcInitialsAvatarColor(user.id)} size={48}>{user.tag}</InitialsAvatar>} badgeAfterTitle={getUserIcons(user, true)} after={getTextTooltip(<Link href={`https://vk.com/im?sel=${user.vkId}`} target="_blank"><IconButton><Icon28MessageOutline /></IconButton></Link>, `Перейти к диалогу`, true)} subtitle={<Link href={`https://vk.com/id${user.vkId}`} target="_blank">vk.com/id{user.vkId}</Link>}>{user.name}</SimpleCell>);
...
const Script = (props) => {
	const { script, getData, getFakeData } = props;
	let { from, to, server, timestamp } = props;
	...
	useEffect(() => {
		...
		const load = async() => {
			try {
				...
				if (!from && to) from = to;
				if (!to && from) to = from;
				if (dataServer) server = dataServer;
	...
	useEffect(() => {
		dataServers&&console.log(dataServers);
	}, [dataServers]);
	useEffect(() => {
		dataStatuses&&console.log(dataStatuses);
	}, [dataStatuses]);
	useEffect(() => {
		dataProfile&&console.log(dataProfile);
	}, [dataProfile]);
	useEffect(() => {
		dataClan&&console.log(dataClan);
	}, [dataClan]);
	useEffect(() => {
		dataFight&&console.log(dataFight);
	}, [dataFight]);
			...
			{loaded && dataProfile ? <>
				{dataServers?.length ? <>
					{dataServers.map((server, key) => <SimpleCell
						onClick={() => {
							console.log(dataServer, server.id);
							setDataServer(server.id);
						}}
						key={key}
				...
				{dataClan && <>
					...
					<details className="dvvDetails">
						...
						<div className="dvvDetails__content">
							...
							<CardGrid size="xs">
								<Card>
									<TextTooltip text={<span>{numberSpaces(dataClan.guard[0] * 50 + 100)} урон стражи</span>} placement={"top"} appearance="inversion">
										<SimpleCell
											disabled
								...
								<Card>
									<TextTooltip text={<span>{numberSpaces(dataClan.guard[1] * 5000 + 1000)} здоровье стражи</span>} placement={"top"} appearance="inversion">
										<SimpleCell
											disabled
								...
								<Card>
									<TextTooltip text={<span>{numberSpaces(dataClan.ram[0] * 50 + 100)} урон тарана</span>} placement={"top"} appearance="inversion">
										<SimpleCell
											disabled
								...
								<Card>
									<TextTooltip text={<span>{numberSpaces(dataClan.ram[1] * 5000 + 1000)} здоровье тарана</span>} placement={"top"} appearance="inversion">
										<SimpleCell
											disabled
					...
					<details className="dvvDetails">
						...
						<div className="dvvDetails__content">
							{[0].map(() => {
								if (!dataClan.actions.treasury.day.length) return (<Footer key={0}>нет пополнений за текущий день</Footer>);
							...
							{[0].map(() => {
								if (!dataClan.actions.treasury.week.length) return (<Footer key={0}>нет пополнений за текущую неделю</Footer>);
