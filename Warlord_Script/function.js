...
function ScriptUpdateServer(name) {
	let script__settings = ScriptUpdateSettings(name);
	document.querySelector(`.script__navigation > [${name}] .script__server`).innerHTML = `${['Эрмун', 'Антарес'][Number(script__settings[name]._9)-1]}`;
}
...
async function ScriptLoad(type, name, text, version) {
	...
	if (type == 'page' && document.querySelector('.script__head') !== null) {
		let script__settings = ScriptUpdateSettings(name);
		let html = `<div class="head__button" ${name}>${text}<span>${version}</span><span class="script__server">${['Эрмун', 'Антарес'][script__settings[name]._9-1]}</span></div>`;
		document.querySelector('.script__navigation').insertAdjacentHTML('beforeend', html);
		document.querySelector(`.script__navigation > [${name}] .script__server`).addEventListener('click', function() {
			if (document.querySelector(`.head__button[${name}]`).getAttribute('disabled') == null) {
				document.querySelector(`.script__settings`).click();
