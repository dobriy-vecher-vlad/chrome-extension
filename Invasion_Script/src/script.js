			...
			try {
				...
				if (settings.showGuild || settings.showGuildUpgrades || settings.showGuildPersonnel || settings.showGuildEvents || settings.showGuildDeposits) {
					if (Number(profile.u.clan_id) != 0) {
						...
						clan = await getData(`https://${server.link}game.php?api_uid=${status.clan_id}&UID=${status.clan_id}&api_type=vk&api_id=${status.api_id}&auth_key=${status.clan_auth}&i=49&t1=${profile.u.clan_id}`);
